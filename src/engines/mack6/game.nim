...
from bitboard import get_bottom_square, get_top_square, split, get_activations
...
proc newMove(
    ...
    if is_castling == 'K':
        ...
        id = "o-o-o"
    elif from_square == 0 and to_square == 0:
        id = "X"
    else:
        id = SQUARES_TO_HUMAN[from_square] & SQUARES_TO_HUMAN[to_square]
        if is_promoting_to != '0':
            id &= $is_promoting_to
    return Move(
        id: id,
        player: player,
...
proc count_legal_moves(game: Game, depth: int = 1): int =
    if depth == 0:
        return 1
...
proc get_input*(game: Game): array[837, float32] =
    result[0] = if game.player: 1'f else: 0'f
    var c = 0
    for a in game.position.pieces['K'].get_activations:
        c += 1
        result[c] = if a == 0: 0'f else: 1'f
    for a in game.position.pieces['Q'].get_activations:
        c += 1
        result[c] = if a == 0: 0'f else: 1'f
    for a in game.position.pieces['R'].get_activations:
        c += 1
        result[c] = if a == 0: 0'f else: 1'f
    for a in game.position.pieces['B'].get_activations:
        c += 1
        result[c] = if a == 0: 0'f else: 1'f
    for a in game.position.pieces['N'].get_activations:
        c += 1
        result[c] = if a == 0: 0'f else: 1'f
    for a in game.position.pieces['P'].get_activations:
        c += 1
        result[c] = if a == 0: 0'f else: 1'f
    for a in game.position.pieces['k'].get_activations:
        c += 1
    ...
    for a in game.position.pieces['p'].get_activations:
        ...
        result[c] = if a == 0: 0'f else: 1'f
    c += 1
    result[c] = if game.possible_castles['K']: 1'f else: 0'f
    c += 1
    result[c] = if game.possible_castles['Q']: 1'f else: 0'f
    c += 1
    result[c] = if game.possible_castles['k']: 1'f else: 0'f
    c += 1
    result[c] = if game.possible_castles['q']: 1'f else: 0'f
    for a in game.en_passant_square.get_activations:
        c += 1
        result[c] = if a == 0: 0'f else: 1'f


proc game_from_fen*(fen: string): Game =
...
var OUTPUT_LAYER_MAPPING* = initTable[string, int]()
var i = 0

for from_square in SQUARES:
    let queen_moves = (
        NORTH_RAY[from_square] or
        SOUTH_RAY[from_square] or
        WEST_RAY[from_square] or
        EAST_RAY[from_square] or
        NORTH_WEST_RAY[from_square] or
        NORTH_EAST_RAY[from_square] or
        SOUTH_WEST_RAY[from_square] or
        SOUTH_EAST_RAY[from_square]
    )
    for to_square in split(queen_moves):
        OUTPUT_LAYER_MAPPING[newMove(true, 'Q', from_square, to_square).id] = i
        i += 1

    for to_square in split(KNIGHT_MOVES[from_square]):
        OUTPUT_LAYER_MAPPING[newMove(true, 'N', from_square, to_square).id] = i
        i += 1

    for to_square in split(PAWN_SINGLE_MOVES[true][from_square] and 0xFF00_0000_0000_0000'u64):
        OUTPUT_LAYER_MAPPING[newMove(true, 'P', from_square, to_square, is_promoting_to='Q').id] = i
        i += 1
        OUTPUT_LAYER_MAPPING[newMove(true, 'P', from_square, to_square, is_promoting_to='R').id] = i
        i += 1
        OUTPUT_LAYER_MAPPING[newMove(true, 'P', from_square, to_square, is_promoting_to='B').id] = i
        i += 1
        OUTPUT_LAYER_MAPPING[newMove(true, 'P', from_square, to_square, is_promoting_to='N').id] = i
        i += 1
    for to_square in split(PAWN_SINGLE_MOVES[false][from_square] and 0x0000_0000_0000_00FF'u64):
        OUTPUT_LAYER_MAPPING[newMove(false, 'P', from_square, to_square, is_promoting_to='Q').id] = i
        i += 1
        OUTPUT_LAYER_MAPPING[newMove(false, 'P', from_square, to_square, is_promoting_to='R').id] = i
        i += 1
        OUTPUT_LAYER_MAPPING[newMove(false, 'P', from_square, to_square, is_promoting_to='B').id] = i
        i += 1
        OUTPUT_LAYER_MAPPING[newMove(false, 'P', from_square, to_square, is_promoting_to='N').id] = i
        i += 1
    for to_square in PAWN_ATTACK_MOVES[true][from_square]:
        if (to_square and 0xFF00_0000_0000_0000'u64) != 0:
            OUTPUT_LAYER_MAPPING[newMove(true, 'P', from_square, to_square, is_promoting_to='Q').id] = i
            i += 1
            OUTPUT_LAYER_MAPPING[newMove(true, 'P', from_square, to_square, is_promoting_to='R').id] = i
            i += 1
            ...
            OUTPUT_LAYER_MAPPING[newMove(true, 'P', from_square, to_square, is_promoting_to='N').id] = i
            i += 1
    for to_square in PAWN_ATTACK_MOVES[false][from_square]:
        if (to_square and 0x0000_0000_0000_00FF'u64) != 0:
            OUTPUT_LAYER_MAPPING[newMove(false, 'P', from_square, to_square, is_promoting_to='Q').id] = i
            i += 1
            OUTPUT_LAYER_MAPPING[newMove(false, 'P', from_square, to_square, is_promoting_to='R').id] = i
            i += 1
            ...
            OUTPUT_LAYER_MAPPING[newMove(false, 'P', from_square, to_square, is_promoting_to='N').id] = i
            i += 1
OUTPUT_LAYER_MAPPING[newMove(true, 'K', 0, 0, is_castling='K').id] = i
i += 1
OUTPUT_LAYER_MAPPING[newMove(true, 'K', 0, 0, is_castling='Q').id] = i
i += 1
OUTPUT_LAYER_MAPPING[newMove(false, 'K', 0, 0, is_castling='k').id] = i
i += 1
OUTPUT_LAYER_MAPPING[newMove(false, 'K', 0, 0, is_castling='q').id] = i
